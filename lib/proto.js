...
exports.remove = function(){
  var args = [].slice.call(arguments),
      fn = args.pop() || noop,
      self = this,
      destroy = this.model.destroy || sync.destroy;

  if (this.isNew()) return fn(new Error('not saved'));

  this.model.emit('removing', this);
  this.emit('removing');
  destroy.apply(this, args.concat(res));
...
exports.save = function(){
  ...
  var args = [].slice.call(arguments),
      fn = args.pop() || noop,
      self = this,
      create = this.model.create || sync.create;

  if (!this.isValid()) return fn(new Error('validation failed'));

  this.model.emit('saving', this);
  this.emit('saving');
  create.apply(this, args.concat(res));

  function res(err, body) {
    if (err) return fn(error(err));
    if (body) self.primary(body.id);
    self.dirty = {};
...
exports.update = function() {
  var args = [].slice.call(arguments),
      fn = args.pop() || noop,
      self = this,
      update = this.model.update || sync.update;

  if (!this.isValid()) return fn(new Error('validation failed'));

  this.model.emit('saving', this);
  this.emit('saving');
  update.apply(this, args.concat(res));
